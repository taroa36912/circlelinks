...
- **LINEログイン:**
  - `firebase_auth_service.dart` にて、モバイル専用 (iOS/Android) のログイン機能を提供します。
  - `flutter_line_sdk` を使用してLINE IDトークンを取得します。
  - `cloud_functions` を呼び出し、バックエンド (Cloud Functions) でIDトークンを検証し、Firebaseカスタムトークンを取得します。
  - `signInWithCustomToken` を使用してFirebaseにサインインします。
- **ログアウト:**
  - `AppMenuDrawer` (サイドメニュー) からログアウトが可能です。

### 2. ナビゲーション
- **メイン画面 (`circle_discovery.dart`):**
  - ログイン後のメイン画面（ホーム）です。
  - 右上の**メニューボタン (3本バー)** をタップすると `AppMenuDrawer` (サイドメニュー) が表示されます。
- **サイドメニュー (`app_menu_drawer.dart`):**
  - アプリの主要機能へのナビゲーションハブとして機能します。
  - ユーザー情報（サークル名/メールアドレス）がヘッダーに表示されます。
  - 以下の画面へ遷移します:
    - 所属サークル一覧 (未実装)
    - Myポートフォリオ
    - コネクション管理
...
### 3. サークル機能 (Circle Features)
- **サークル登録 (`circle_registration.dart`):**
  - 新規ユーザー（サークル管理者）がアカウント作成と同時にサークル情報を登録する、3ステップのページャー形式のフォームです。
  - **ステップ1 (アカウント):** メール/パスワード設定。
  - **ステップ2 (基本情報):** 大学名、サークル名、カテゴリ、説明、メンバー数。
  - **ステップ3 (詳細情報):** プロフィール画像、カバー画像、公認証明書、SNSリンク。
  - Web/モバイル両対応の画像アップロード機能 (`File` / `Uint8List`) を備えます。
- **サークル一覧 (発見) (`circle_discovery.dart`):**
  - Firestoreからサークル一覧をストリームで取得し、`ListView` で表示します。
  - **検索:** `SearchBarWidget` によるキーワード検索。
  - **カテゴリタブ:** `TabBar` によるカテゴリ絞り込み。
  - **フィルタリング:** FAB (フローティングアクションボタン) から `FilterModalWidget` を呼び出し、詳細な絞り込みが可能です。
  - **ソート:** `SortButtonWidget` による並び替え。
- **サークル詳細プロフィール (`circle_profile.dart`):**
  - サークル一覧から特定のサークルをタップすると遷移する詳細画面です。
  - カバー画像をヘッダーに持つ `SliverAppBar` を使用します。
  - **タブ:** 「About」（基本情報、活動内容、SNSリンク）と「Contact」（連絡先情報）を表示します。
  - **アクション:** FABから「コネクションリクエスト」を送信できます。

### 4. ソーシャル機能 (Connections & Chat)
- **コネクション管理 (`connections.dart`):**
  - サークル間の接続を管理します。
  - **タブ:** 「受信リクエスト」と「接続済みサークル」に分かれています。
  - 受信したリクエストの「承認」または「却下」が可能です。
  - 接続済みのサークルをタップするとチャット画面に遷移します。
- **チャット (`chat.dart`):**
  - 接続済みのサークルと1対1でチャットを行う画面です。
  - Firestoreの `messages` サブコレクションからメッセージをストリームで取得し、送信者によって左右に振り分けて表示します。

### 5. イベント機能 (Event Management)
- **イベント作成 (`event_creation.dart`):**
  - 新しいイベントを作成するための詳細なフォーム画面です。（`widgets` フォルダに6つのサブウィジェットあり）
  - 日時、場所、出欠、支払い、詳細オプション（写真アルバムなど）を設定可能です。
  - カテゴリに基づく「おすすめ設定」の提案機能や、作成前のプレビュー機能があります。
  - 現在は `_createEvent` での実際のFirestoreへの保存はモックです。
- **イベント詳細 (`event_details.dart`):**
  - イベントの詳細情報を表示する画面です。（現在は**モックデータ**で動作しています）
  - `CustomScrollView` を使用し、ヘッダーにイベント画像を表示します。
  - 出欠状況、参加者一覧、写真、費用、コメントなどを表示します。

### 6. ユーザー機能 (User Features)
- **ポートフォリオビルダー (`portfolio_builder.dart`):**
  - ユーザー（サークルメンバー個人）の活動実績やスキルをまとめる画面です。（現在は**モックデータ**で動作しています）
  - **タブ:** 「Portfolio」（役職、イベント、プロジェクト）と「Skills」（スキルと推薦）に分かれています。
  - 各実績の編集、追加、検証リクエスト（モック）が可能です。
  - ポートフォリオのエクスポート機能（モック）への導線があります。